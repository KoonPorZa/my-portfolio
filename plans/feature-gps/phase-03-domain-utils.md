...
4. `client.ts` — `uploadLocation(point, { token })` (HTTPS POST to the Fastify backend from Phase 11, using `NEXT_PUBLIC_TRIP_GPS_API_BASE`) plus a retry/offline-queue **skeleton** (interface + no-op queue for now; real queue is Phase 8).
...
## Out of scope
- Network endpoint implementation and persistence (Phase 11), real offline queue (Phase 8).
