...
5. Poll `GET /api/trips/001/location` every 30–60s (use `nextPollMs` from the response). Stop or slow polling when the session is `stopped` or the token is invalid.
...
## Out of scope
- Owner controls (Phase 2), token issuance/revoke internals (Phase 7), heavy map deps.
