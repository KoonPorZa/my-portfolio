...
5. Periodic capture using cadence constants from `lib/trip-gps/cadence.ts`: active 5m, saver 10m, rest 15m. Send immediately on Start, Stop, and Manual ping. Use the upload client from Phase 3 (`lib/trip-gps/client.ts`).
...
- [ ] Stop clears timer/watch/wake-lock and triggers session stop (Phase 4/7).
- [ ] Mobile-responsive; matches the light roadbook theme; `lint` + `build` pass.

## Out of scope
- Backend persistence, token validation, viewer page (Phases 4–7).
